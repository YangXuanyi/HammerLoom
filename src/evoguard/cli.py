...
def database_path(value: Optional[str]) -> str:
    return value or ".evoguard/evoguard.db"


def main() -> None:
    parser = argparse.ArgumentParser(prog="evoguard", description="Evidence-driven evolution control plane for coding agents")
    parser.add_argument("--db", help="SQLite database path (default: .evoguard/evoguard.db)")
    sub = parser.add_subparsers(dest="command", required=True)
    init = sub.add_parser("init", help="Initialize local EvoGuard state")
